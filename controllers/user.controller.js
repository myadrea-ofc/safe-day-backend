...
exports.createUser = async (req, res) => {
  try {
    ...
    const finalSiteId = req.user.role === 'superadmin' ? site_id : req.user.site_id;

    if (!finalSiteId) {
      return res.status(400).json({ message: 'Site wajib diisi' });
    }

    if (req.user.role === 'admin' && role === 'admin' && finalSiteId !== req.user.site_id) {
      return res.status(403).json({
        message: 'Admin hanya boleh membuat admin di site sendiri',
      });
    }

    const roleRes = await pool.query('SELECT id FROM roles WHERE role_name=$1', [role]);
    if (roleRes.rowCount === 0) {
      return res.status(400).json({ message: 'Role tidak valid' });
    }

    const deptCheck = await pool.query('SELECT 1 FROM departments WHERE id=$1 AND site_id=$2', [
      department_id,
      finalSiteId,
    ]);
    ...
    const result = await pool.query(
      ...
      `,
      [name, hashed, roleRes.rows[0].id, finalSiteId, department_id, req.user.id, emailNormalized],
    );
