...
router.post(
  "/",
  authMiddleware,
  auditMiddleware("P2H Water Truck"),
  (req, res) => {
...
try{
    ...
    return res.status(201).json({
        ...
        console.error("P2H Water Truck POST ERROR:", e);
        return res.status(500).json({
          success: false,
          error: e.message,
        });
...
// ===================== GET =====================
router.get("/", authMiddleware, auditMiddleware("P2H Water Truck"), async (req, res) => {
  try {
    const { role, site_id } = req.user;

    let query = `SELECT * FROM p2h_water_truck`;
    const params = [];
    ...
    console.error("P2H WATER TRUCK GET ERROR:", err);
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("P2H Water Truck"),
  ensureExcelDownloadAccess("p2h_water_truck"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_WATER_TRUCK_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Water Truck Export");
      ...
      titleCell.value = "LAPORAN EXPORT P2H WATER TRUCK";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        for (let j = 0; j < 5; j++) {
          ...
          const fileCell = excelRow.getCell(59 + j);
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_water_truck"],
      );
    } catch (err) {
      console.error("P2H WATER TRUCK EXPORT XLSX ERROR:", err);
