...
router.post(
  "/",
  authMiddleware,
  auditMiddleware("P2H Water Pump"),
  (req, res) => {
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("P2H Water Pump"),
  ensureExcelDownloadAccess("p2h_water_pump"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      let query = `
        SELECT
          ...
          p.alat_keselamatan_air1,
          p.alat_keselamatan_air2,
          p.alat_keselamatan_air3,
          p.alat_keselamatan_air4,
      ...
      const exportFileName = `P2H_WATER_PUMP_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Water Pump Export");
      ...
      worksheet.columns = [
        ...
        { key: "alat_keselamatan_air1", width: 30 },
        { key: "alat_keselamatan_air2", width: 30 },
        { key: "alat_keselamatan_air3", width: 30 },
        { key: "alat_keselamatan_air4", width: 30 },
      ...
      titleCell.value = "LAPORAN EXPORT P2H WATER PUMP";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        const excelRow = worksheet.addRow([
          ...
          row.alat_keselamatan_air1 ?? "-",
          row.alat_keselamatan_air2 ?? "-",
          row.alat_keselamatan_air3 ?? "-",
          row.alat_keselamatan_air4 ?? "-",
        ...
        for (let j = 0; j < 5; j++) {
          ...
          const fileCell = excelRow.getCell(37 + j);
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_water_pump"],
      );
    } catch (err) {
      console.error("P2H WATER PUMP EXPORT XLSX ERROR:", err);
