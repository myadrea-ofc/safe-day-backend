...
router.post("/", authMiddleware, auditMiddleware("P2H Dozer"), (req, res) => {
  upload.array("files", 5)(req, res, async (err) => {
    ...
    try {
      ...
      const query = `
        INSERT INTO p2h_dozer (
          nama, nrp, jabatan, department, perusahaan, lokasi_kerja,
          no_lambung_unit, hm_unit, waktu, brand_unit,
      ...
      console.error("P2H DOZER POST ERROR:", e);
      return res.status(500).json({
        success: false,
        error: e.message,
      });
...
// ===================== GET =====================
router.get("/", authMiddleware, auditMiddleware("P2H Dozer"), async (req, res) => {
  try {
    const { role, site_id } = req.user;

    let query = `SELECT * FROM p2h_dozer`;
    const params = [];
    ...
    console.error("P2H DOZER GET ERROR:", err);
...
router.get(
  "/export.xlsx",
  authMiddleware,
  auditMiddleware("P2H Dozer"),
  ensureExcelDownloadAccess("p2h_dozer"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_DOZER_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Dozer Export");
      ...
      titleCell.value = "LAPORAN EXPORT P2H DOZER";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      const colorableColumns = [
        ...
        43, 44, 45, 46, 48, 49, 50, 51, 52, 53, 54,
      ];
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_dozer"],
      );
    } catch (err) {
      console.error("P2H DOZER EXPORT XLSX ERROR:", err);
