...
// ===================== POST =====================
router.post("/", authMiddleware, auditMiddleware("P2H Crane"), (req, res) => {
    ...
    try {
      ...
      const query = `
        INSERT INTO p2h_crane (
          ...
          opsi_item31, opsi_item32, opsi_item33, opsi_item34, opsi_item35,
          opsi_item36,
...
// ===================== GET =====================
router.get("/", authMiddleware, auditMiddleware("P2H Crane"), async (req, res) => {
  try {
    ...
    console.error("P2H CRANE GET ERROR:", err);
    return res.status(500).json({ message: "Server error" });
  }
});
...
router.get(
  "/export.xlsx",
  authMiddleware,
  auditMiddleware("P2H Crane"),
  ensureExcelDownloadAccess("p2h_crane"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_CRANE_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Crane Export");
      ...
      worksheet.columns = [
        ...
        { key: "opsi_item35", width: 45 },
        { key: "opsi_item36", width: 45 },
      ...
      titleCell.value = "LAPORAN EXPORT P2H CRANE";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_crane"],
      );
    } catch (err) {
      console.error("P2H CRANE EXPORT XLSX ERROR:", err);
