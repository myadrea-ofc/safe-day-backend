...
router.post(
  "/",
  authMiddleware,
  auditMiddleware("P2H Wheel Loader"),
  (req, res) => {
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    let query = `SELECT * FROM p2h_wheelloader`;
    const params = [];
    ...
    console.error("P2H WHEELLOADER GET ERROR:", err);
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("P2H Wheel Loader"),
  ensureExcelDownloadAccess("p2h_wheelloader"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_WHEELLOADER_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Wheel Loader Export");
      ...
      titleCell.value = "LAPORAN EXPORT P2H WHEEL LOADER";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_wheelloader"],
      );
    } catch (err) {
      console.error("P2H WHEELLOADER EXPORT XLSX ERROR:", err);
