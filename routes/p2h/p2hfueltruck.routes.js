...
router.post("/", authMiddleware, auditMiddleware("P2H Fuel Truck"), (req, res) => {
    ...
    try {
      ...
      const query = `
        INSERT INTO p2h_fuel_truck (
          ...
          opsi_item31, opsi_item32,

          opsi_standar_keselamatan1, opsi_standar_keselamatan2, opsi_standar_keselamatan3,
      ...
      console.error("P2H FUEL TRUCK POST ERROR:", e);
      return res.status(500).json({
        success: false,
        error: e.message,
      });
...
// ===================== GET =====================
router.get("/", authMiddleware, auditMiddleware("P2H Fuel Truck"), async (req, res) => {
  try {
    const { role, site_id } = req.user;

    let query = `SELECT * FROM p2h_fuel_truck`;
    const params = [];
    ...
    console.error("P2H FUEL TRUCK GET ERROR:", err);
...
router.get(
  "/export.xlsx",
  authMiddleware,
  auditMiddleware("P2H Fuel Truck"),
  ensureExcelDownloadAccess("p2h_fuel_truck"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_FUEL_TRUCK_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Fuel Truck Export");
      ...
      titleCell.value = "LAPORAN EXPORT P2H FUEL TRUCK";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        for (let j = 0; j < 5; j++) {
          ...
          const fileCell = excelRow.getCell(57 + j);
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_fuel_truck"],
      );
    } catch (err) {
      console.error("P2H FUEL TRUCK EXPORT XLSX ERROR:", err);
