...
// ===================== GET =====================
router.get("/", authMiddleware, auditMiddleware("P2H Forklift"), async (req, res) => {
  try {
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("P2H Forklift"),
  ensureExcelDownloadAccess("p2h_forklift"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_FORKLIFT_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Forklift Export");
      ...
      titleCell.value = "LAPORAN EXPORT P2H FORKLIFT";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_forklift"],
      );
    } catch (err) {
      console.error("P2H FORKLIFT EXPORT XLSX ERROR:", err);
