...
router.post("/", authMiddleware, auditMiddleware("P2H Compactor"), (req, res) => {
  upload.array("files", 5)(req, res, async (err) => {
    ...
    try {
      ...
      console.error("P2H COMPACTOR POST ERROR:", e);
      return res.status(500).json({
        success: false,
        error: e.message,
      });
...
router.get("/", authMiddleware, auditMiddleware("P2H Compactor"), async (req, res) => {
  try {
    ...
    let query = `SELECT * FROM p2h_compactor`;
    const params = [];
    ...
    console.error("P2H COMPACTOR GET ERROR:", err);
...
router.get(
  "/export.xlsx",
  authMiddleware,
  auditMiddleware("P2H Compactor"),
  ensureExcelDownloadAccess("p2h_compactor"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const exportFileName = `P2H_COMPACTOR_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("P2H Compactor Export");
      ...
      titleCell.value = "LAPORAN EXPORT P2H COMPACTOR";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "p2h_compactor"],
      );
    } catch (err) {
      console.error("P2H COMPACTOR EXPORT XLSX ERROR:", err);
