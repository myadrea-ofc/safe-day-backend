...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      infoCell.value = `Generated By: ${req.user.name || req.user.id} | Role: ${req.user.role} | Site: ${currentUserSiteName} | Generated At: ${generatedAtText}`;
      infoCell.font = {
        italic: true,
        size: 11,
        color: { argb: "FF374151" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        const siapKerjaValue = (row.siap_kerja || "")
          .toString()
          .toLowerCase()
          .trim();
