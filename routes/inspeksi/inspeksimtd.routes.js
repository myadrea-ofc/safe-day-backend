...
router.post(
  "/",
  authMiddleware,
  auditMiddleware("Inspeksi Mess Toilet & Dapur"),
  upload.fields([
  ...
  async (req, res) => {
    try {
      ...
      return res.status(201).json({
        success: true,
        message: "Data Inspeksi MTD berhasil disimpan",
        id: result.rows[0].id,
      });
    } catch (error) {
      console.error("INSPEKSI MTD POST ERROR:", error);
...
router.get("/", authMiddleware, auditMiddleware("Inspeksi Mess Toilet & Dapur"), async (req, res) => {
  try {
    ...
    console.error("INSPEKSI MTD GET ERROR:", err);
    return res.status(500).json({ message: "Server error" });
  }
});
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("Inspeksi Mess Toilet & Dapur"),
  ensureExcelDownloadAccess("inspeksi_mtd"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const fileName = `INSPEKSI_MTD_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("Inspeksi MTD Export");
      ...
      titleCell.value = "LAPORAN EXPORT INSPEKSI MTD";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "inspeksi_mtd"],
      );
    } catch (err) {
      console.error("INSPEKSI MTD EXPORT XLSX ERROR:", err);
