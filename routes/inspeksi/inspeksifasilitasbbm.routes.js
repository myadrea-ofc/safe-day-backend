...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const query = `
        INSERT INTO inspeksi_fasilitas_bbm (
          nama,
          nrp,
          department,
          perusahaan,
      ...
      return res.status(201).json({
        success: true,
        message: "Data Inspeksi Fasilitas BBM berhasil disimpan",
        id: result.rows[0].id,
      });
    } catch (error) {
      console.error("INSPEKSI FASILITAS BBM POST ERROR:", error);
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("INSPEKSI FASILITAS BBM GET ERROR:", err);
    return res.status(500).json({ message: "Server error" });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      const lastExport = await pool.query(
        ...
        LIMIT 1
        `,
        [req.user.id, "inspeksi_fasilitas_bbm"],
      );
      ...
      const fileName = `INSPEKSI_FASILITAS_BBM_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("Inspeksi Fasilitas BBM Export");
      ...
      titleCell.value = "LAPORAN EXPORT INSPEKSI FASILITAS BBM";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        for (let j = 0; j < 4; j++) {
          ...
          const fotoCell = excelRow.getCell(34 + j);
        ...
        const statusCell = excelRow.getCell(31);
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "inspeksi_fasilitas_bbm"],
      );
    } catch (err) {
      console.error("INSPEKSI FASILITAS BBM EXPORT XLSX ERROR:", err);
