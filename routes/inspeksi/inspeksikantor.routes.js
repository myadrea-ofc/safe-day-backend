...
router.post(
  "/",
  authMiddleware,
  auditMiddleware("Inspeksi Kantor"),
  upload.fields([
  ...
  async (req, res) => {
    try {
      ...
      return res.status(201).json({
        success: true,
        message: "Data Inspeksi Kantor berhasil disimpan",
        id: result.rows[0].id,
      });
    } catch (error) {
      console.error("INSPEKSI KANTOR POST ERROR:", error);
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("INSPEKSI KANTOR GET ERROR:", err);
    return res.status(500).json({ message: "Server error" });
  }
});
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("Inspeksi Kantor"),
  ensureExcelDownloadAccess("inspeksi_kantor"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const fileName = `INSPEKSI_KANTOR_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("Inspeksi Kantor Export");
      ...
      titleCell.value = "LAPORAN EXPORT INSPEKSI KANTOR";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        for (let j = 0; j < 4; j++) {
          ...
          const fotoCell = excelRow.getCell(35 + j);
        ...
        const statusCell = excelRow.getCell(32);
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "inspeksi_kantor"],
      );
    } catch (err) {
      console.error("INSPEKSI KANTOR EXPORT XLSX ERROR:", err);
