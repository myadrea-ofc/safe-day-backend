...
router.post(
  ...
  async (req, res) => {
    try {
      console.log("=== INSPEKSI JALAN TAMBANG ===");
...
INSERT INTO inspeksi_jalan_tambang (
  ...
  ket_hasil, saran_masuk, status_inspeksi, apar,
  foto1, foto2, foto3,
  site_id
)
VALUES (
...
`;
  ...
  site_id
      ...
      console.error("INSPEKSI JALAN TAMBANG ERROR:", error);
      res.status(500).json({
        success: false,
        error: error.message,
      });
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("INSPEKSI GET ERROR:", err);
