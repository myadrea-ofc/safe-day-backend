...
router.post(
  "/",
  authMiddleware,
  auditMiddleware("Inspeksi Jalan Tambang"),
  upload.fields([
  ...
  async (req, res) => {
    try {
      ...
      const query = `
        INSERT INTO inspeksi_jalan_tambang (
          nama,
          nrp,
          department,
          perusahaan,
      ...
      return res.status(201).json({
        success: true,
        message: "Data Inspeksi Jalan Tambang berhasil disimpan",
        id: result.rows[0].id,
      });
    } catch (error) {
      console.error("INSPEKSI JALAN TAMBANG POST ERROR:", error);
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("INSPEKSI JALAN TAMBANG GET ERROR:", err);
    return res.status(500).json({ message: "Server error" });
  }
});
...
router.get(
  ...
  authMiddleware,
  auditMiddleware("Inspeksi Jalan Tambang"),
  ensureExcelDownloadAccess("inspeksi_jalan_tambang"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const lastExport = await pool.query(
        ...
        LIMIT 1
        `,
        [req.user.id, "inspeksi_jalan_tambang"],
      );
      ...
      const fileName = `INSPEKSI_JALAN_TAMBANG_${formattedDate}.xlsx`;
      ...
      const worksheet = workbook.addWorksheet("Inspeksi Jalan Tambang Export");
      ...
      titleCell.value = "LAPORAN EXPORT INSPEKSI JALAN TAMBANG";
      titleCell.font = {
        bold: true,
        size: 16,
        color: { argb: "FFFFFFFF" },
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        for (let j = 0; j < 3; j++) {
          ...
          const fotoCell = excelRow.getCell(36 + j);
        ...
        const statusCell = excelRow.getCell(33);
      ...
      await pool.query(
        ...
        VALUES ($1, $2, $3)
        `,
        [req.user.id, req.user.site_id, "inspeksi_jalan_tambang"],
      );
    } catch (err) {
      console.error("INSPEKSI JALAN TAMBANG EXPORT XLSX ERROR:", err);
