...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error("INSPEKSI CHP POST ERROR:", error);
...
router.get(
  "/export.xlsx",
  authMiddleware,
  ensureExcelDownloadAccess("inspeksi_chp"),
  async (req, res) => {
    try {
      const { role, site_id } = req.user;
      ...
      const worksheet = workbook.addWorksheet("Inspeksi CHP Export");
      ...
      for (let i = 0; i < result.rows.length; i++) {
        ...
        const statusCell = excelRow.getCell(30);
